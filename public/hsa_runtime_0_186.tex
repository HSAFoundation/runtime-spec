...
parts. First, the \reffld{type} field in the packet header must be set to the
appropriate value:
\begin{lstlisting}
dispatch_packet->header.type = HSA_PACKET_TYPE_DISPATCH;
\end{lstlisting}
Second, the buffer index where the packet has been written (in the example, zero)
...
\item Launch the packet by first setting the \reffld{type} field on its header
  ...
  variant that uses a different memory order).
...
\vspace{-5mm}
\subsection{Example - a simple dispatch}\vspace{-1mm}
