...
typedef enum {
    ...
    /**
     * Indicates an error occurred, specifics were either not
     * determinable or not encoded in the error list.
     */
    HSA_STATUS_FAILURE,
...
typedef struct _hsa_async_error_info_s{
    /**
     * Indicates the type of the error, based on this, the user knows
     * if and packet_id is available in one of the reserved words.
     */
    hsa_status_t error_type;

    /**
     * The queue that processed the entity that caused the
     * asynchronous error.
     */
    uint32_t     queue_id;

    /**
     * A pointer to more information, this could be pointing to
     * implementation specific details that could be useful to some
     * tools or to binary data
     */
    void         *info;

    /**
     * ISO/IEC 646 character encoding must be used. A string
     * indicating some error information. The string should be NUL
     * terminated per ISO 646.
     */
    char         *string_info;
...
/** \addtogroup register_notify
 *  @{
 */
/**
 * @brief Register a notification callback.
 *
 * @param[in] callback The callback that the user is
 *      registering, which will be invoked with info as a parameter.
 *
 * @param[in] user_data The user data to call the callback with. The
 *      @a user_data field of @a info will be filled with value when
 *      the callback is called.
 *
 * @param[in] context Identifies a particular runtime context that
 *      this callback is registered for. When a callback is registered
 *      for a particular context, it will only be invoked if the
 *      notification is for an action in that context.
 *
 * @retval ::HSA_STATUS_SUCCESS If successful
 *
 ...
 * for context @a c1.
 *
 * @param[in] callback The callback that the user is registering, the
 *      callback is called with info structure. User can read the
 *      structure and access its elements.
 *
 * @param[in] user_data The user data to call the callback
 *      with. info.user_data will be filled with value when the
 *      callback is called.
 *
 * @param[in] context The runtime context that this callback is being
 *      registered for.
 *
 * @retval ::HSA_STATUS_SUCCESS If successful
 *
 ...
 * considered successful in that ::HSA_STATUS_SUCCESS is returned with
...
/**
 * @brief Increment reference count of a context, if it is not
 * currently zero.
 *
 * @param[in] input_context The context that the user is explicitly
 *      reference counting. User allocated.
 *
 * @retval ::HSA_STATUS_SUCCESS If successful.
 *
 * @retval ::HSA_STATUS_ERROR_NOT_INITIALIZED If invoked before the
...
/** \defgroup memory_descriptor TODO
 *  @{
 */
/**
...
/** \defgroup cache_descriptor TODO
 *  @{
 */
/**
...
typedef struct hsa_cache_descriptor_s{
    ...
    /**
     * Is the cache inclusive with the level above? The array has
     * size @a levels - 1.
     */
...
typedef struct hsa_agent_s{
    ...
    /**
     * Array of memory descriptor offsets.  Number of elements in
     * array equals @a number_memory_descriptors.
    */
    ...
    /**
     * Array of cache descriptor offsets.  Number of elements in array
     * equals @a number_cache_descriptors.
    */
 ...
 *      input, the time in component clock frequency, this can be obtained by
 ...
 * replace, if the comparison was successful. User allocated.
 */
 ...
 * @param[in] value User defined value.
 *
 * @param[out] min_value Minimum of @a value and the signal's current
 * value.
 *
 * @retval ::HSA_STATUS_SUCCESS If successful.
 *
...
typedef struct hsa_queue_s{
  ...
  /**
   * A pointer to another User Mode Queue that can be used by the
   * HSAIL kernel to request system services. The serviceQueue
   * property is provided by the application to the runtime API call
   * when the queue is created, and may be NULL, the system provided
   * serviceQueue or an application managed queue.
   */
...
typedef struct hsa_aql_packet_header_s {
  /**
   * 8 bits for describing the packet type, 0 for INVALID, 1 for
   * COMPONENT DISPATCH, 2 for BARRIER and 4 for AGENT DISPATCH. All
...
typedef struct hsa_aql_agent_dispatch_packet_s{
  ...
  /**
   * The function to be performed by the destination HSA Agent. The
   * type value is split into the following ranges: 0x0000:0x3FFF
   * (vendor specific), 0x4000:0x7FFF (HSA runtime) 0x8000:0xFFFF
...
typedef enum {
  /**
   * Do not return a service queue, the service queue pointer will be
   * set to NULL by the runtime
   */
  NONE=0,
  ...
  /**
   * Create a queue that runtime will not manage, the application has
   * to manage the queue manually.
   */
  APPLICATION_MANAGED
...
/** \defgroup queue_inactivate TODO
 *  @{
 */
/**
...
typedef struct hsa_symbol_map_s{
    ...
    /**
     * for implementations to store other information and allow for
     * queries
     */
    uint64_t reserved;
} hsa_symbol_map_t;
...
typedef struct hsa_control_directives_s {
  /**
   ...
   and the rest of the fields can be ignored. The bits are accessed
   using the hsa_control_directives_present_mask_t. Any control
   directive that is not enabled in this bit set must have the value
   of all 0s.
   */
  ...
  /**
   ...
   @a enabled_control_directives, then must be
  ...
  /**
   If enable detect exceptions is not enabled in @a
   enabled_control_directives, then must be 0, otherwise must be non-0
   and specifies the set of HSAIL exceptions that must have the DETECT
   policy enabled.  If the kernel being finalized has any
   enabledetectexceptions control directives, then the values
   specified by this argument are unioned with the values in these
   control directives. If any of the functions the kernel calls have
   an enabledetectexceptions control directive, then they must be
   equal or a subset of, this union.
   */
  hsa_exception_kind16_t enable_detect_exceptions;

  /**
   If max dynamic group size is not enabled in @a
   enabled_control_directives then this must be 0, and any amount of
   dynamic group segment can be allocated for a dispatch, otherwise
   the value specifies the maximum number of bytes of dynamic group
   segment that can be allocated for a dispatch. If the kernel being
   finalized has any maxdynamicsize control directives, then the
   values must be the same, and must be the same as this argument if
   it is enabled. This value can be used by the finalizer to determine
   the maximum number of bytes of group memory used by each work-group
   by adding this value to the group memory required for all group
   segment variables used by the kernel and all functions it calls,
   and group memory used to implement other HSAIL features such as
   fbarriers and the detect exception operations. This can allow the
   finalizer to determine the expected number of work-groups that can
   be executed by a compute unit and allow more resources to be
   allocated to the work-items if it is known that fewer work-groups
   can be executed due to group memory limitations.
   */
  uint32_t max_dynamic_group_size;

  /**
   If this is not enabled in @a enabled_control_directives then
   must be 0, otherwise must be greater than 0.  See HSA Programmer's
  ...
  /**
   If this is not enabled in @a enabled_control_directives then
   must be 0 and the finalizer may generate ISA that could result in
   any number of work-groups executing on a single compute unit.
   Otherwise, the finalizer will attempt to generate ISA that will
   allow the specified number of work-groups to execute on a single
   compute unit. This is only a hint and can be ignored by the
   finalizer. If the kernel being finalized, or any of the functions
   it calls, has the same control directive, then the values must be
   the same or the finalization can fail. This can be used to
   determine the number of resources that should be allocated to a
   single work-group and work-item.
   */
  ...
  /**
   ...
   consistent with required_dim and
   max_flat_workgroup_size. If the kernel
...
typedef struct hsa_code_s {
  /**
   The code format version. The version of this definition is
...
typedef struct hsa_kernel_code_s{
  ...
  */
  ...
  /**
   ...
   fbarriers then that amount must already be included in the @a
...
typedef struct hsa_brig_s{
    /**
     * From PRM: string section, containing all character strings and
     * byte data used in the compilation unit.
     */
    uint8_t *string_section;

    /**
     * The directives, which provide information for the
     * finalizer. The directives do not generate code.
     */
    uint8_t *directive_section;

    /**
     * All of the executable operations. Most operations contain
     * offsets to the .operand section.
     */
    uint8_t *code_section;

    /**
     * The operands, such as immediate constants, registers, and
     * address expressions, that appear in the operations.
     */
...
typedef struct hsa_compilationunit_code_s{
  ...
  /**
   A bit set of flags providing information about the code in this
 ...
 * @brief Finalize BRIG
 *
 * @details The input pointer must reference an in memory location of
 * the BRIG. Also, the BRIG must contain at the minimum the 4 basic
 * BRIG sections (.strings, .directives, .code, .operands). All
 * symbols in the symbol/offset table must be resolved.
 *
 * @param caller
 ...
 *      input. a pointer to the kernel code object.
 * @param brig
 *      input. the brig this symbol table came from.
 * @param symbol_name
 ...
 * @param brig
 *      the brig this symbol table came from
 ...
 * @brief TODO
 *
 * @param symbol_map input. destroys the symbol map resources
 *      including any additional structures
 */
 ...
 *
 * @details Registering a system memory region for use with all the
 * available devices This is an optional interface that is solely
 * provided as a performance optimization hint to the underlying
 * implementation so it may prepare for the future use of the memory
 * by the devices. The interface is only beneficial for system memory
 * that will be directly accessed by a device.
 *
 * Overlapping registrations are allowed. This is neither detrimental
 * nor beneficial.
 *
 ...
 *
 * @details The returned buffer is already registered. Allocation of
 ...
 *
 * @details Deallocate component memory that was
...
/**
 * @brief Image handle, populated by ::hsa_create_image_handle. Images
...
typedef enum  {
    ...
    */
    ...
    /**
    * Images of this format are guaranteed to have consistent data
    * layout regardless of the how it is accessed by the HSA agent.
    */
...
/**
 * @brief Agent-specific image size and alignment
 * requirements. This structure stores the agent-dependent image
...
/**
 * @brief Defines how the HSA device expects to access the image. The
 * access pattern used by the HSA agent specified in
 * ::hsa_create_image_handle
 *
 */
typedef enum {
  /**
   * Image handle is to be used by the HSA agent as read-only using an
   * HSAIL roimg type.
   */
  HSA_AGENT_IMAGE_ACCESS_READ_ONLY,

  /**
   * Image handle is to be used by the HSA agent as write-only using
   * an HSAIL woimg type.
   */
  HSA_AGENT_IMAGE_ACCESS_WRITE_ONLY,

  /**
   * Image handle is to be used by the HSA agent as read and/or write
   * using an HSAIL rwimg type.
   */
...
/**
 * @brief Geometry associated with the HSA image (image dimensions
 * allowed in HSA). The enumeration values match the HSAIL BRIG type
 * BrigImageGeometry.
 */
typedef enum {
/**
  ...
  /**
   * Array of two-dimensional depth images with the same size and format.
   * 2D arrays are addressed by index and width and height coordinates.
   */
...
/**
* @brief Component type associated with the image. See Image section
* in HSA Programming Reference Manual for definitions on each
* component type. The enumeration values match the HSAIL BRIG type
...
/**
 *
* @brief Image component order associated with the image. See Image
* section in HSA Programming Reference Manual for definitions on each
* component order. The enumeration values match the HSAIL BRIG type
 ...
 * ::hsa_create_sampler_handle. Sampler handles are only unique within
 * an agent, not across agents.
 */
 ...
 * the processing of out-of-range image coordinates.The values match
 * the HSAIL BRIG type BrigSamplerAddressing.
 */
typedef enum {
...
/**
 * @brief Sampler coordinate modes. The enumeration values match the
 * HSAIL BRIG BRIG_SAMPLER_COORD bit in the type BrigSamplerModifier
 */
typedef enum {
  /**
...
/**
 * @brief Sampler filter modes. The enumeration values match the HSAIL
 * BRIG type BrigSamplerFilter.
 */
typedef enum {
  /**
...
typedef struct hsa_sampler_descriptor_s {
  /**
   * Sampler coordinate mode describes the normalization of image
   * coordinates
   */
  ...
  /**
   * Sampler address mode describes the processing of out-of-range
   * image coordinates
   */
...
/**
 * @brief Retrieve image format capabilities for the specified image format on the specified HSA component.
 *
 * @details If successful, the queried image format's capabilities
 * bit-mask is written to the location specified by @a
 * capability_mask. See ::hsa_image_format_capability_t to determine
 * all possible capabilities that can be reported in the bit mask.
 *
 ...
 *       If @a agent, @a image_format, or @a capability_mask are NULL.
 */
hsa_status_t hsa_get_image_format_capability(const hsa_agent_t *agent,
                         const hsa_image_format_t *image_format,
                         hsa_image_geometry_t image_geometry,
                         uint32_t *capability_mask);
/**
 *
 * @brief Inquires the required HSA component-specific image data
 * details from a implementation independent image descriptor.
 *
 * @details If successful, the function the queried HSA agent-specific
 * image data info is written to the location specified by @a
 * image_info. Based on the implementation the optimal image data size
 * and alignment requirements could vary depending on the image
 * attributes specified in @a image_descriptor.
 *
 * @param[in] agent HSA agent to be associated with the image.
 *
 * @param[in] image_descriptor Implementation-independent image
 * descriptor describing the image.
 *
 * @param[out] image_info Image info size and alignment requirements
 * that the HSA agent requires.
 *
 * @retval ::HSA_STATUS_SUCCESS If successful.
 *
 * @retval ::HSA_STATUS_ERROR_INVALID_ARGUMENT If any of the arguments
 * is NULL.
 *
 * @retval ::HSA_STATUS_UNSUPPORTED_IMAGE_FORMAT If the HSA agent does
 * not support the image format specified by the descriptor.
 *
 * @retval ::HSA_STATUS_IMAGE_SIZE_NOT_SUPPORTED If the HSA agent does
 * not support the image dimensions specified by the format
 * descriptor.
 */
 ...
 * implementation-independent image descriptor and a
 * agent-specific image data. The image access defines how the HSA
 * agent expects to use the image and must match the HSAIL image
 * handle type used by the agent.
 *
 * @details If successful, the image handle is written to the location
 * specified by @a image_handle. The image data memory must be
 * allocated using the previously queried ::hsa_get_image_info memory
 * requirements with the same HSA agent and implementation-independent
 * image descriptor.
 *
 * The image data is not initialized and any previous memory contents
 * is preserved. The memory management of image data is the
 * application's responsibility and can only be freed until the memory
 * is no longer needed and any image handles using it are destroyed.
 *
 * @a agent_access defines how the HSA agent expects to use the image
 * handle. The image format specified in @a image_descriptor must be
 * capable by the HSA agent for the intended access pattern.
 *
 * Image handles with different agent_access can be created using the
 * same image data with exactly the same image descriptor as long as
 * ::HSA_IMAGE_FORMAT_ACCESS_INVARIANT_IMAGE_DATA is reported by
 * ::hsa_get_image_format_capability for the image format specified in
 * the image descriptor. Images of non-linear SRGB channel order types
 * can share the same image data with its equivalent linear RGB
 * channel order types, provided the rest of image descriptor
 ...
 *
 * @param[in] agent_access Access pattern of the image by the HSA
 * agent.
 *
 ...
 *
 * @retval ::HSA_STATUS_UNSUPPORTED_IMAGE_FORMAT if the HSA agent does
 * not have the capability to support the image format using the
 * specified @a agent_access.
 *
...
/**
 * @brief Imports a linearly organized image data from memory directly
 * to an image handle.
 *
 * @details This operation updates the image data referenced by the
 * image handle from the source memory. The size of the data imported
 * from memory is implicitly derived from the image region.
 *
 * If @a completion_signal is NULL, the operation occurs
 * synchronously. Otherwise the function returns immediately and the
 * @a completion_signal is signaled when the operation completes.
 *
 * If @a src_row_pitch is smaller than the destination region width
 * (in bytes), then @a src_row_pitch = region width.
 *
 * If @a src_slice_pitch is smaller than the destination region width
 * * region height (in bytes), then @a src_slice_pitch = region width *
 * region height.
 *
 * It is the application's responsibility to avoid out of bounds
 * memory access.
 *
 * None of the source memory or image data memory in the previously
 * created ::hsa_create_image_handle image handle can overlap.
 * Overlapping of any of the source and destination memory within the
 * import operation produces undefined results.
 *
 * @param[in] agent HSA agent to be associated with the image.
 *
 * @param[in] src_memory Source memory.
 *
 * @param[in] src_row_pitch Number of bytes in one row of the source
 * memory.
 *
 * @param[in] src_slice_pitch Number of bytes in one slice of the
 * source memory.
 *
 ...
 *
 * @param[in] completion_signal Signal to set when the operation is
 * completed.
 *
 * @retval ::HSA_STATUS_SUCCESS If successful.
 *
 * @retval ::HSA_STATUS_ERROR_INVALID_ARGUMENT If @a agent, @a
 * src_memory or @a image_region are NULL.
 *
 */
...
/**
 * @brief Export image data from the image handle directly to memory
 * organized linearly.
 *
 * @details The operation updates the destination memory with the
 * image data in the image handle. The size of the data exported to
 * memory is implicitly derived from the image region.
 *
 * If @a completion_signal is NULL, the operation occurs
 * synchronously. Otherwise the function returns immediately and the
 * @a completion_signal is signaled when the operation completes.
 *
 * If @a dst_row_pitch is smaller than the source region width (in
 * bytes), then @a dst_row_pitch = region width.
 *
 * If @a dst_slice_pitch is smaller than the source region width *
 * region height (in bytes), then @a dst_slice_pitch = region width *
 * region height.
 *
 * It is the application's responsibility to avoid out of bounds
 * memory access.
 *
 * None of the destination memory or image data memory in the
 * previously created ::hsa_create_image_handle image handle can
 * overlap – overlapping of any of the source and destination memory
 * within the export operation produces undefined results.
 *
 * @param[in] agent HSA agent to be associated with the image.
 *
 ...
 *
 * @param[in] dst_row_pitch Number of bytes in one row of the
 * destination memory.
 *
 * @param[in] dst_slice_pitch Number of bytes in one slice of the
 * destination memory.
 *
 ...
 *
 * @retval ::HSA_STATUS_ERROR_INVALID_ARGUMENT If @a agent, @a
 * dst_memory or @a image_region are NULL.
 */
 ...
 *
 * @details The operation copies the image data from the source image
 * handle to the destination image handle. The size of the image data
 * copied is implicitly derived from the image region.
 *
 * If @a completion_signal is NULL, the operation occurs
 * synchronously. Otherwise the function returns immediately and the
 * @a completion_signal is signaled when the operation completes.
 ...
 * memory access.
 *
 * The source and destination handles must have been previously
 * created using ::hsa_create_image_handle. The source and destination
 * image data memory are not allowed to be the same. Overlapping any
 * of the source and destination memory produces undefined results.
 *
 * The source and destination image formats don't have to match;
 * appropriate format conversion is performed automatically. The
 ...
 *
 * @details The operation clears the elements of the image with the
 * data specified. The lowest bits of the data (number of bits
 * depending on the image component type) are stored in the cleared
 * image are based on the image component order. The size of the image
 * data cleared is implicitly derived from the image region.
 *
 * If @a completion_signal is NULL, the operation occurs
 * synchronously. Otherwise the function returns immediately and the
 * @a completion_signal is signaled when the operation completes.
 ...
 * memory access.
 *
 * Clearing an image automatically performs value conversion on the
 * provided floating point values as is appropriate for the image
 * format used.
 *
 * For images of UNORM types, the floating point values must be in the
 * [0..1] range. For images of SNORM types, the floating point values
 * must be in the [-1..1] range. For images of UINT types, the
 * floating point values are rounded down to an integer value. For
 * images of SRGB types, the clear data is specified in a linear
 * space, which is appropriately converted by the Runtime to sRGB
 * color space.
 *
 * Specifying clear value outside of the range representable by an
 * image format produces undefined results.
 *
 * @param[in] agent HSA agent to be associated with the image.
 *
 ...
 *  using ::hsa_create_image_handle is destroyed.
 *
 * Destroying the image handle does not free the associated image
 * data.
 *
 ...
 * dispatch. Failure to properly track image data lifetime causes
 * undefined results due to premature image handle deletion.
 *
 * @param[in] agent HSA agent to be associated with the image.
 *
 ...
 *
 * @details If successful, the sampler handle is written to the
 * location specified by the sampler handle.
 *
 * @param[in] agent HSA agent to be associated with the image.
 *
 * @param[in] sampler_descriptor Implementation-independent sampler
 * descriptor
 *
 ...
 * is NULL.
 *
 * @retval ::HSA_STATUS_ERROR_OUT_OF_RESOURCES if the HSA agent cannot
 * create the specified handle because it is out of resources.
 */
 ...
 * ::hsa_create_sampler_handle is destroyed.
 *
 * The sampler handle should not be destroyed while there are
 * references to it queued for execution or currently being used in a
 * dispatch.
 *
 * @param[in] agent HSA agent to be associated with the image.
